...
describe('CollectionTemplateEditionNFTs', () => {
  ...
  before(async () => {
    ...
    // deploy the contract
    await contract.init(collectionTemplateData.name, collectionTemplateData.symbol, 8);

    // init and deploy receiver contract
    const receiver_contract_source = utils.getContractContent(RECEIVER_CONTRACT_SOURCE);
    receiver_contract = await aeSdk.initializeContract({ sourceCode: receiver_contract_source, fileSystem: utils.getFilesystem(RECEIVER_CONTRACT_SOURCE) });
    await receiver_contract.init();
  });
  ...
  describe('NFT collection', async () => {
    ...
    it('template_limit', async () => {
      const { decodedResult } = await contract.template_limit();
  ...
  describe('Template creation', async () => {
    it('failed template creation', async () => {;
      await expect(
        contract.create_template({'MetadataIdentifier': ["ipfs://..."]}, -1, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_INVALID"`);
      await expect(
        contract.create_template({'MetadataIdentifier': ["ipfs://..."]}, 0, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_INVALID"`);
      await expect(
        contract.create_template({'MetadataMap': [new Map()]}, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "METADATA_TYPE_NOT_ALLOWED"`);
      await expect(
        contract.create_template({'MetadataIdentifier': ["ipfs://..."]}, undefined, { onAccount: accounts[1] }))
    ...
    it('template creation', async () => {;
      for(let i=0; i<collectionTemplateData.templates.length; i++) {
        const createTemplateTx =
          await contract.create_template(
      ...
      // check template
      let dryRunTemplate = await contract.template(0);
      assert.equal(dryRunTemplate.decodedResult, undefined);

      // check template_supply
      let dryRunTemplateCount = await contract.template_supply();
      assert.equal(dryRunTemplateCount.decodedResult, 8);

      for(let i=0; i<collectionTemplateData.templates.length; i++) {
        dryRunTemplate = await contract.template(i+1);
  ...
  describe('NFT specific interactions', async () => {
    it('failed minting - basic check', async () => {
      ...
      await expect(
        contract.template_mint(address, 0, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_ID_NOT_EXISTS"`);
      await expect(
        contract.template_mint(address, 1, undefined, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_CONTRACT_OWNER_CALL_ALLOWED"`);
    });
    ...
    it('minting', async () => {
      ...
      for(let i=0; i<collectionTemplateData.templates.length; i++) {
        for(let j=0; j<collectionTemplateData.templates[i].edition_limit; j++) {
          const templateMintTx = await contract.template_mint(address, i+1, undefined, { onAccount: accounts[0] });
          totalMintCount++;
      ...
      metadataDryRun = await contract.metadata(totalMintCount);
    ...
    it('failed minting - edition limit check', async () => {
      ...
      for(let i=0; i<8; i++) {
        await expect(
          contract.template_mint(address, i+1, undefined, { onAccount: accounts[0] }))
    ...
    it('transfer', async () => {
      ...
      const transferTx = await contract.transfer(to, totalMintCount, { onAccount: accounts[0] });
    ...
    it('failed burn', async () => {
      await expect(
        contract.burn(0, { onAccount: accounts[0] }))
    ...
    it('burn', async () => {
      ...
      // burn again
      burnTx = await contract.burn(totalMintCount, { onAccount: accounts[1] });
      assert.equal(burnTx.decodedEvents[0].name, 'Burn');
    ...
    it('failed update of mutable metadata', async () => {
      ...
      await expect(
        contract.update_mutable_attributes(1, updatedMutableAttributes, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_CONTRACT_OWNER_CALL_ALLOWED"`);
      await expect(
        contract.update_mutable_attributes(1, updatedMutableAttributes, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TOKEN_NOT_EXISTS"`);
    });
    ...
    it('update mutable metadata', async () => {
      ...
      let dryRunMetadata = await contract.metadata(2);
      assert.equal(dryRunMetadata.decodedResult.MetadataMap[0].get('mutable_attributes'), undefined);

      // update mutable attributes
      const updateMutableAttributesTx = await contract.update_mutable_attributes(2, updatedMutableAttributes, { onAccount: accounts[0] });
      ...
      dryRunMetadata = await contract.metadata(2);
  ...
  describe('Template interactions', async () => {
    ...
    it('template creation, limit & deletion', async () => {
      const template_level_contract = await aeSdk.initializeContract({ sourceCode, fileSystem });
      // deploy contract
      await template_level_contract.init('template_level_example', 'TLE', 2);

      // check template supply
      let dryRunTemplateSupply = await template_level_contract.template_supply();
      assert.equal(dryRunTemplateSupply.decodedResult, 0);

      // create two templates
      await template_level_contract.create_template({'MetadataIdentifier': ['url_placeholder1']}, undefined, { onAccount: accounts[0] });
      await template_level_contract.create_template({'MetadataIdentifier': ['url_placeholder2']}, undefined, { onAccount: accounts[0] });

      // should not be possible to create another template
      await expect(
        template_level_contract.create_template({'MetadataIdentifier': ['url_placeholder3']}, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_LIMIT_REACHED"`);
      
      // check template supply
      dryRunTemplateSupply = await template_level_contract.template_supply();
      assert.equal(dryRunTemplateSupply.decodedResult, 2);

      // delete template
      let deleteTemplateTx = await template_level_contract.delete_template(1, { onAccount: accounts[0] });
      ...
      await expect(
        template_level_contract.delete_template(1, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_ID_NOT_EXISTS"`);
      ...
      let dryRunTemplate = await template_level_contract.template(1);
      assert.equal(dryRunTemplate.decodedResult, undefined);

      // create a new templat, should be possible after deletion
      const createTemplateTx = await template_level_contract.create_template({'MetadataIdentifier': ['url_placeholder3']}, undefined, { onAccount: accounts[0] });
      ...
      // check total supply
      let dryRunTotalSupply = await template_level_contract.total_supply();
      assert.equal(dryRunTotalSupply.decodedResult, 0);

      // mint an NFT for template with id 3
      await template_level_contract.template_mint(accounts[0].address, 3, undefined, { onAccount: accounts[0] });

      // check total supply
      dryRunTotalSupply = await template_level_contract.total_supply();
      ...
      await expect(
        template_level_contract.delete_template(3, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_DELETION_NOT_ALLOWED"`);
    });

    it('template limit changes', async () => {
      const template_limit_contract = await aeSdk.initializeContract({ sourceCode, fileSystem });
      // deploy contract
      await template_limit_contract.init("template_limit_contract_example", "TECE", 3);

      // create three templates
      await template_limit_contract.create_template({'MetadataIdentifier': ['url_placeholder1']}, undefined, { onAccount: accounts[0] });
      await template_limit_contract.create_template({'MetadataIdentifier': ['url_placeholder2']}, undefined, { onAccount: accounts[0] });
      await template_limit_contract.create_template({'MetadataIdentifier': ['url_placeholder3']}, undefined, { onAccount: accounts[0] });

      // should not be possible to create another template
      await expect(
        template_limit_contract.create_template({'MetadataIdentifier': ['url_placeholder4']}, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_LIMIT_REACHED"`);

      // check various conditions for changing the template limit
      await expect(
        template_limit_contract.decrease_template_limit(4, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TEMPLATE_LIMIT_INCREASE_NOT_ALLOWED"`);
      await expect(
        template_limit_contract.decrease_template_limit(3, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "NO_CHANGE_PROVIDED"`);
      await expect(
        template_limit_contract.decrease_template_limit(2, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "MORE_TEMPLATES_ALREADY_IN_EXISTENCE"`);

      // delete a template
      await template_limit_contract.delete_template(1, { onAccount: accounts[0] });

      // after deleting a template it should be possible to decrease the limit
      const changeTemplateLimitTx = await template_limit_contract.decrease_template_limit(2, { onAccount: accounts[0] });
    ...
    it('edition level', async () => {
      const edition_level_contract = await aeSdk.initializeContract({ sourceCode, fileSystem });
      // deploy contract
      await edition_level_contract.init("edition_level_example", "TLE", 10);

      // expect creation of template with invalid edition limit to fail
      await expect(
        edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder1']}, -1, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_INVALID"`);
      await expect(
        edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder1']}, 0, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_INVALID"`);
      
      // creation of template with unlimited edition size
      let createTemplateTx = await edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder1']}, undefined, { onAccount: accounts[0] });
      ...
      for(let i=0; i<5; i++) {
        await edition_level_contract.template_mint(accounts[0].address, 1, undefined, { onAccount: accounts[0] });
      }
      // check template
      let dryRunTemplate = await edition_level_contract.template(1);
      ...
      createTemplateTx = await edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder2']}, 1, { onAccount: accounts[0] });
      ...
      await edition_level_contract.template_mint(accounts[0].address, 2, undefined, { onAccount: accounts[0] });
      // expect another mint of the template to fail
      await expect(
        edition_level_contract.template_mint(accounts[0].address, 2, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_EXCEEDED"`);
      // check template
      dryRunTemplate = await edition_level_contract.template(2);
      assert.deepEqual(dryRunTemplate.decodedResult,
        {
          immutable_metadata: {
      ...
      await edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder3']}, 3, { onAccount: accounts[0] });
      // mint two NFTs based on template id 3
      await edition_level_contract.template_mint(accounts[0].address, 3, undefined, { onAccount: accounts[0] });
      await edition_level_contract.template_mint(accounts[0].address, 3, undefined, { onAccount: accounts[0] });
      // check template
      dryRunTemplate = await edition_level_contract.template(3);
      assert.deepEqual(dryRunTemplate.decodedResult,
        {
          immutable_metadata: {
      ...
      await edition_level_contract.create_template({'MetadataIdentifier': ['url_placeholder4']}, undefined, { onAccount: accounts[0] });
      // check template
      dryRunTemplate = await edition_level_contract.template(4);
      assert.deepEqual(dryRunTemplate.decodedResult,
        {
          immutable_metadata: {
      ...
      await edition_level_contract.template_mint(accounts[0].address, 4, undefined, { onAccount: accounts[0] });
      await edition_level_contract.template_mint(accounts[0].address, 4, undefined, { onAccount: accounts[0] });
      await edition_level_contract.template_mint(accounts[0].address, 4, undefined, { onAccount: accounts[0] });
      // check template
      dryRunTemplate = await edition_level_contract.template(4);
      ...
      const changeEditionLimitTx = await edition_level_contract.decrease_edition_limit(4, 3, { onAccount: accounts[0] });
      ...
      await expect(
        edition_level_contract.template_mint(accounts[0].address, 4, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_EXCEEDED"`);
      
      // expect failed change of edition limit
      await expect(
        edition_level_contract.decrease_edition_limit(4, 4, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "EDITION_LIMIT_INCREASE_NOT_ALLOWED"`);
      await expect(
        edition_level_contract.decrease_edition_limit(4, 3, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "NO_CHANGE_PROVIDED"`);
      await expect(
        edition_level_contract.decrease_edition_limit(4, 2, { onAccount: accounts[0] }))

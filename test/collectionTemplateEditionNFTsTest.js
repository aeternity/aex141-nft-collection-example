...
describe('CollectionTemplateEditionNFTs', () => {
  ...
  describe('NFT collection', async () => {
    it('aex141_extensions', async () => {
      ...
      assert.deepEqual(decodedResult, ['mintable_templates', 'mutable_attributes', 'burnable', 'swappable']);
    });
    it('meta_info', async () => {
  ...
  describe('NFT specific interactions', async () => {
    ...
    it('swap', async () => {
      ...
      assert.equal(swapTx.decodedEvents.length, totalMintCount - 2);
      const toSwap = new Set(allTokensToBeMinted.slice(1, -1));
      for(let i=0; i<totalMintCount-2; i++) {
      ...
      assert.deepEqual(checkSwapDryRun.decodedResult, allTokensToBeMinted.slice(1, -1));

      // check swapped
      const swappedDryRun = await contract.methods.swapped();
      assert.deepEqual(swappedDryRun.decodedResult, new Map([[await accounts[0].address(), toSwap]]));

...
describe('CollectionUniqueNFTs', () => {
  ...
  before(async () => {
    ...
    // deploy the contract
    const contractCreateTx = await contract.init(
      collectionUniqueMetadata.name,
      collectionUniqueMetadata.symbol,
      8);
    ...
    receiverContract = await aeSdk.initializeContract({ sourceCode: receiverContractSource, fileSystem: utils.getFilesystem(RECEIVER_CONTRACT_SOURCE) });
    await receiverContract.init();
  });
  ...
  describe('NFT collection', async () => {
    ...
    it('token_limit', async () => {
      const { decodedResult } = await contract.token_limit();
  ...
  describe('NFT specific interactions', async () => {
    it('failed minting', async () => {
      ...
      await expect(
        contract.mint(address, undefined, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "NO_METADATA_PROVIDED"`);
      await expect(
        contract.mint(address, {'MetadataIdentifier': ['fails anyway ...']}, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "NOT_METADATA_MAP"`);
      await expect(
        contract.mint(address, {'MetadataMap': [new Map()]}, undefined, { onAccount: accounts[1] }))
    ...
    it('minting', async () => {
      ...
      const getOwnedTokensDryRun = await contract.get_owned_tokens(address);
    ...
    it('failed mint due to minting_limit', async () => {
      ...
      await expect(
        contract.mint(accounts[0].address, {'MetadataMap': [new Map()]}, undefined, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TOKEN_LIMIT_REACHED"`);
    });

    it('failed decrease_token_limit', async () => {
      await expect(
        contract.decrease_token_limit(5, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_CONTRACT_OWNER_CALL_ALLOWED"`);
      await expect(
        contract.decrease_token_limit(9, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TOKEN_LIMIT_INCREASE_NOT_ALLOWED"`);
      await expect(
        contract.decrease_token_limit(8, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "NO_CHANGE_PROVIDED"`);
      await expect(
        contract.decrease_token_limit(7, { onAccount: accounts[0] }))
    ...
    it('transfer', async () => {
      ...
      await expect(
        contract.transfer(from, 8, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "SENDER_MUST_NOT_BE_RECEIVER"`);
      
      // expect successful transfer
      const to = accounts[1].address;
      const transferTx = await contract.transfer(to, 8, { onAccount: accounts[0] });
      ...
      let balanceDryRun = await contract.balance(from);
      assert.equal(balanceDryRun.decodedResult, 7);
      balanceDryRun = await contract.balance(to);
      assert.equal(balanceDryRun.decodedResult, 1);

      // check owned NFTs after transfer
      let getOwnedTokensDryRun = await contract.get_owned_tokens(from);
      assert.deepEqual(getOwnedTokensDryRun.decodedResult, [1n,2n,3n,4n,5n,6n,7n]);
      getOwnedTokensDryRun = await contract.get_owned_tokens(to);
      assert.deepEqual(getOwnedTokensDryRun.decodedResult, [8n]);
    });

    it('failed burn', async () => {
      await expect(
        contract.burn(10, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "TOKEN_NOT_EXISTS"`);
      await expect(
        contract.burn(1, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);
    });

    it('burn', async () => {
      // burn
      let burnTx = await contract.burn(1, { onAccount: accounts[0] });
      assert.equal(burnTx.decodedEvents[0].name, 'Burn');
      assert.equal(burnTx.decodedEvents[0].args[0], accounts[0].address);
      assert.equal(burnTx.decodedEvents[0].args[1], 1);

      // burn again
      burnTx = await contract.burn(8, { onAccount: accounts[1] });
      assert.equal(burnTx.decodedEvents[0].name, 'Burn');
      assert.equal(burnTx.decodedEvents[0].args[0], accounts[1].address);
      ...
      // check owned NFTs after transfer
      let getOwnedTokensDryRun = await contract.get_owned_tokens(accounts[0].address);
      assert.deepEqual(getOwnedTokensDryRun.decodedResult, [2n,3n,4n,5n,6n,7n]);
      getOwnedTokensDryRun = await contract.get_owned_tokens(accounts[1].address);
      assert.deepEqual(getOwnedTokensDryRun.decodedResult, []);

      // check total supply after burn
      const totalSupplyDryRun = await contract.total_supply();
      assert.equal(totalSupplyDryRun.decodedResult, 6);
    });

    it('approval & operator (simple accounts)', async () => {
      // check get_approved
      let getApprovedDryRun = await contract.get_approved(2);
      assert.equal(getApprovedDryRun.decodedResult, undefined);

      // check is approved
      let isApprovedDryRun = await contract.is_approved(2, accounts[1].address);
      assert.equal(isApprovedDryRun.decodedResult, false);

      // approve other account
      let approveTx = await contract.approve(accounts[1].address, 2, true, { onAccount: accounts[0] });
      assert.equal(approveTx.decodedEvents[0].name, 'Approval');
      assert.equal(approveTx.decodedEvents[0].args[0], accounts[0].address);
      assert.equal(approveTx.decodedEvents[0].args[1], accounts[1].address);
      assert.equal(approveTx.decodedEvents[0].args[2], 2);
      assert.equal(approveTx.decodedEvents[0].args[3], "true");

      // check get_approved
      getApprovedDryRun = await contract.get_approved(2);
      assert.equal(getApprovedDryRun.decodedResult, accounts[1].address);

      // check is approved
      isApprovedDryRun = await contract.is_approved(2, accounts[1].address);
      assert.equal(isApprovedDryRun.decodedResult, true);

      // check is approved for all
      let isApprovedForAllDryRun = await contract.is_approved_for_all(accounts[1].address, accounts[2].address);
      assert.equal(isApprovedForAllDryRun.decodedResult, false);

      // approve all
      let approveAllTx = await contract.approve_all(accounts[2].address, true, { onAccount: accounts[1] });
      assert.equal(approveAllTx.decodedEvents[0].name, 'ApprovalForAll');
      assert.equal(approveAllTx.decodedEvents[0].args[0], accounts[1].address);
      assert.equal(approveAllTx.decodedEvents[0].args[1], accounts[2].address);
      assert.equal(approveAllTx.decodedEvents[0].args[2], "true");

      // check is approved for all
      isApprovedForAllDryRun = await contract.is_approved_for_all(accounts[1].address, accounts[2].address);
      assert.equal(isApprovedForAllDryRun.decodedResult, true);

      // check owner
      let ownerDryRun = await contract.owner(2);
      assert.equal(ownerDryRun.decodedResult, accounts[0].address);

      // transfer as approved account to the approved account
      let transferTx = await contract.transfer(accounts[1].address, 2, undefined, { onAccount: accounts[1] });
      assert.equal(transferTx.decodedEvents[0].name, 'Transfer');
      assert.equal(transferTx.decodedEvents[0].args[0], accounts[0].address);
      assert.equal(transferTx.decodedEvents[0].args[1], accounts[1].address);
      ...
      transferTx = await contract.transfer(accounts[1].address, 3, undefined, { onAccount: accounts[0] });
      transferTx = await contract.transfer(accounts[1].address, 4, undefined, { onAccount: accounts[0] });
      ...
      let burnTx = await contract.burn(2, { onAccount: accounts[2] });
      assert.equal(burnTx.decodedEvents[0].name, 'Burn');
      assert.equal(burnTx.decodedEvents[0].args[0], accounts[1].address);
      ...
      // check total supply
      let totalSupplyDryRun = await contract.total_supply();
      assert.equal(totalSupplyDryRun.decodedResult, 5);

      // transfer NFT from acc 1 back to acc 0 via operator (acc 2)
      transferTx = await contract.transfer(accounts[0].address, 3, undefined, { onAccount: accounts[2] });

      // check failed burn of token id 3 owned by acc 0 as operator (acc 2) for acc 1
      await expect(
        contract.burn(3, { onAccount: accounts[2] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);

      // remove operator rights
      approveAllTx = await contract.approve_all(accounts[2].address, false, { onAccount: accounts[1] });
      assert.equal(approveAllTx.decodedEvents[0].name, 'ApprovalForAll');
      assert.equal(approveAllTx.decodedEvents[0].args[0], accounts[1].address);
      assert.equal(approveAllTx.decodedEvents[0].args[1], accounts[2].address);
      ...
      await expect(
        contract.transfer(accounts[1].address, 4, { onAccount: accounts[2] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);

      // transfer token id 4 back to acc 0
      transferTx = await contract.transfer(accounts[0].address, 4, undefined, { onAccount: accounts[1] });

      // check final balance of acc 0
      balanceDryRun = await contract.balance(accounts[0].address);
    ...
    it('approval & operator (contracts example)', async () => {
      ...
      const operatorContract = await aeSdk.initializeContract({ sourceCode: operatorContractSource, fileSystem: utils.getFilesystem(OPERATOR_CONTRACT_SOURCE) });
      await operatorContract.$deploy([]);

      const operatorContractId = operatorContract.$options.address;
      const operatorContractAddress = operatorContractId.replace("ct_", "ak_");

      // check get_approved
      let getApprovedDryRun = await contract.get_approved(3);
      assert.equal(getApprovedDryRun.decodedResult, undefined);

      // check is approved
      let isApprovedDryRun = await contract.is_approved(3, operatorContractAddress);
      assert.equal(isApprovedDryRun.decodedResult, false);

      // approve other account
      let approveTx = await contract.approve(operatorContractAddress, 3, true, { onAccount: accounts[0] });
      assert.equal(approveTx.decodedEvents[0].name, 'Approval');
      assert.equal(approveTx.decodedEvents[0].args[0], accounts[0].address);
      ...
      // check get_approved
      getApprovedDryRun = await contract.get_approved(3);
      assert.equal(getApprovedDryRun.decodedResult, operatorContractAddress);

      // check is approved
      isApprovedDryRun = await contract.is_approved(3, operatorContractAddress);
      assert.equal(isApprovedDryRun.decodedResult, true);

      // check is approved for all
      let isApprovedForAllDryRun = await contract.is_approved_for_all(accounts[1].address, operatorContractAddress);
      assert.equal(isApprovedForAllDryRun.decodedResult, false);

      // approve all
      let approveAllTx = await contract.approve_all(operatorContractAddress, true, { onAccount: accounts[1] });
      assert.equal(approveAllTx.decodedEvents[0].name, 'ApprovalForAll');
      assert.equal(approveAllTx.decodedEvents[0].args[0], accounts[1].address);
      ...
      // check is approved for all
      isApprovedForAllDryRun = await contract.is_approved_for_all(accounts[1].address, operatorContractAddress);
      assert.equal(isApprovedForAllDryRun.decodedResult, true);

      // check owner
      let ownerDryRun = await contract.owner(3);
      assert.equal(ownerDryRun.decodedResult, accounts[0].address);

      // trigger transfer from acc 0 to acc 1 via operator contract
      let operatorTransferTx = await operatorContract.trigger_transfer(contract.$options.address, 3, accounts[1].address, undefined);
      assert.equal(operatorTransferTx.decodedEvents[0].name, 'Transfer');
      assert.equal(operatorTransferTx.decodedEvents[0].args[0], accounts[0].address);
      assert.equal(operatorTransferTx.decodedEvents[0].args[1], accounts[1].address);
      ...
      // transfer another 2 NFTs from acc 0 to acc 1
      let transferTx = await contract.transfer(accounts[1].address, 4, undefined, { onAccount: accounts[0] });
      transferTx = await contract.transfer(accounts[1].address, 5, undefined, { onAccount: accounts[0] });
      ...
      let operatorBurnTx = await operatorContract.trigger_burn(contract.$options.address, 3);
      assert.equal(operatorBurnTx.decodedEvents[0].name, 'Burn');
      assert.equal(operatorBurnTx.decodedEvents[0].args[0], accounts[1].address);
      ...
      operatorTransferTx = await operatorContract.trigger_transfer(contract.$options.address, 4, accounts[0].address, undefined);

      // check failed burn of token id 4 owned by acc 0 as operator (acc 2) for acc 1
      await expect(
        operatorContract.trigger_burn(contract.$options.address, 4))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);

      // remove operator rights
      approveAllTx = await contract.approve_all(operatorContractAddress, false, { onAccount: accounts[1] });
      assert.equal(approveAllTx.decodedEvents[0].name, 'ApprovalForAll');
      assert.equal(approveAllTx.decodedEvents[0].args[0], accounts[1].address);
      assert.equal(approveAllTx.decodedEvents[0].args[1], operatorContractAddress);
      assert.equal(approveAllTx.decodedEvents[0].args[2], "false");
      ...
      // check failed transfer from acc 1 back to acc 0 via revoked operator (acc 2)
      await expect(
        operatorContract.trigger_transfer(contract.$options.address, 5, accounts[0].address, undefined))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);

      // transfer token id 5 back to acc 0
      transferTx = await contract.transfer(accounts[0].address, 5, undefined, { onAccount: accounts[1] });
    ...
    it('nft receiver example', async () => {
      const receiverContractId = receiverContract.$options.address;
      const receiverContractAddress = receiverContractId.replace("ct_", "ak_");

      // failed transfer
      await expect(
        contract.transfer(receiverContractAddress, 4, 'FAILS', { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "SAFE_TRANSFER_FAILED"`);

      // check original token owner of specific nft
      await expect(
        receiverContract.get_nft_owner(contract.$options.address, 4))
        .to.be.rejectedWith(`Invocation failed: "UNKNOWN_TOKEN_CONTRACT"`);

      // successful transfer
      let transferTx = await contract.transfer(receiverContractAddress, 4, undefined, { onAccount: accounts[0] });
      assert.equal(transferTx.decodedEvents[0].name, 'Transfer');
      assert.equal(transferTx.decodedEvents[0].args[0], accounts[0].address);
      ...
      const expectedContractToTokenOwnerResult =
        new Map([[contract.$options.address, new Map([[4n, accounts[0].address]])]]);
      
      // check storage of original token owners
      let getContractToTokenownerDryRun = await receiverContract.get_contract_to_tokenowner();
      assert.deepEqual(getContractToTokenownerDryRun.decodedResult,
        expectedContractToTokenOwnerResult);

      // check original token owner of specific nft
      let getNftOwnerDryRun = await receiverContract.get_nft_owner(contract.$options.address, 4);
      assert.equal(getNftOwnerDryRun.decodedResult, accounts[0].address);

      // failed claim as wrong owner
      await expect(
        receiverContract.claim_nft(contract.$options.address, 4, { onAccount: accounts[1] }))
        .to.be.rejectedWith(`Invocation failed: "CLAIM_DENIED_NOT_OWNER"`);

      // claim token back
      const claimNftTx = await receiverContract.claim_nft(contract.$options.address, 4, { onAccount: accounts[0] });
      ...
      assert.equal(claimNftTx.decodedEvents[0].args[1], accounts[0].address);
      assert.equal(claimNftTx.decodedEvents[0].args[2], 4);

      // check original token owner of specific nft
      getNftOwnerDryRun = await receiverContract.get_nft_owner(contract.$options.address, 4);
      assert.equal(getNftOwnerDryRun.decodedResult, undefined);

      // check storage of original token owners
      getContractToTokenownerDryRun = await receiverContract.get_contract_to_tokenowner();
      assert.deepEqual(getContractToTokenownerDryRun.decodedResult, new Map([[contract.$options.address, new Map()]]));
    });
    ...
    it('claim nft from contract', async () => {
      ...
      await expect(
        contract.transfer_to_contract(4, { onAccount: accounts[0] }))
      ...
      await expect(
        receiverContract.transfer_nft_to_contract(contract.$options.address, 4, { onAccount: accounts[0] }))
        .to.be.rejectedWith(`Invocation failed: "ONLY_OWNER_APPROVED_OR_OPERATOR_CALL_ALLOWED"`);

      // approve the receiverContract to control the NFT
      await contract.approve(receiverContractAddress, 4, true, { onAccount: accounts[0] });
      ...
      let transferTx = await receiverContract.transfer_nft_to_contract(contract.$options.address, 4, { onAccount: accounts[5] });
      assert.equal(transferTx.decodedEvents[0].name, 'Transfer');
      assert.equal(transferTx.decodedEvents[0].args[0], accounts[0].address);
      assert.equal(transferTx.decodedEvents[0].args[1], receiverContractAddress);
      assert.equal(transferTx.decodedEvents[0].args[2], 4);
    ...
    it('decrease_token_limit after burns', async () => {
      ...
      const changeTokenLimitTx = await contract.decrease_token_limit(5, { onAccount: accounts[0] });
      ...
      // mint a new token
      await contract.mint(accounts[0].address, {'MetadataMap': [new Map()]}, undefined, { onAccount: accounts[0] });
